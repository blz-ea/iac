...
resource "null_resource" "provision" {
  ...
  triggers = {
	...
	provisioner "local-exec" {
		command = "ansible-playbook -i '${local.node_hostname},' ../modules/terraform/lxc_jellyfin/append.yml -e 'container_name=${local.container_name}' -e 'ansible_user=${local.node_username}' -e 'container_id=${proxmox_virtual_environment_container.container.id}'"
		environment = {
			ANSIBLE_CONFIG = "../ansible.cfg",
			ANSIBLE_FORCE_COLOR = "True"
		}
	...
	# Provision Container
	provisioner "local-exec" {
		command = "ansible-playbook -i '${data.consul_keys.container.var.ipv4_address_0},' ../modules/terraform/lxc_jellyfin/provision.yml -e 'ansible_user=${lookup(var.data, "username", "root")}'"
		environment = {
			ANSIBLE_CONFIG = "../ansible.cfg",
			ANSIBLE_FORCE_COLOR = "True"
		}
